...
function isValidSubdomain(domain) {
    ...
    if (!regex.test(cleanDomain)) {
        return {
            valid: false,
            message:
                'Format subdomain tidak valid! Gunakan huruf, angka, titik, underscore, dan dash.',
        };
    }
    ...
    if (cleanDomain.includes('..') || cleanDomain.includes('--')) {
        return {
            valid: false,
            message: 'Subdomain tidak boleh mengandung titik atau dash berturut-turut',
        };
    }
    ...
    if (
        ...
        return {
            valid: false,
            message: 'Subdomain tidak boleh dimulai atau diakhiri dengan dash atau titik',
        };
    }
...
function isValidApiToken(token) {
    ...
    return typeof token === 'string' && token.trim().length >= 10 && token.trim().length <= 200;
}
...
function isValidDomain(domain) {
    ...
    if (parts.length < 2) {
        return {
            valid: false,
            message:
                'Format domain tidak valid! Domain harus memiliki minimal 2 bagian (contoh: domain.com)',
        };
    }
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        if (!part || part.length === 0) {
            return {
                valid: false,
                message: 'Format domain tidak valid! Tidak boleh ada bagian kosong.',
            };
        }
        ...
        if (part.length > 63) {
            return {
                valid: false,
                message: 'Format domain tidak valid! Setiap bagian domain maksimal 63 karakter.',
            };
        }
        ...
        if (!partRegex.test(part)) {
            return {
                valid: false,
                message:
                    'Format domain tidak valid! Gunakan huruf, angka, dan dash. Tidak boleh dimulai/diakhiri dengan dash.',
            };
        }
    }
    ...
    if (!/^[a-zA-Z]{2,}$/.test(tld)) {
        return {
            valid: false,
            message: 'Format domain tidak valid! TLD harus minimal 2 huruf (contoh: .com, .id)',
        };
    }
