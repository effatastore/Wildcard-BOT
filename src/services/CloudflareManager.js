...
class CloudflareManager {
     ...
     */
    constructor(userId) {
        console.log(`🔧 Initializing CloudflareManager for user ID: ${userId}`);
        this.userId = userId;
    ...
    initializeConfig() {
        ...
        if (!config[this.userId]) {
            throw new Error(`❌ Configuration not found for user ID: ${this.userId}`);
        }
        ...
        if (!apiKey || !credentials.email || !credentials.accountId) {
            throw new Error(
                '❌ Incomplete configuration. Make sure Global API Key, Email, and Account ID are set.'
            );
        }
    ...
    extractMainDomain(domain) {
        ...
        console.log(`[INFO] Processing domain: ${domain}`);
        console.log(`[INFO] Domain parts: ${JSON.stringify(parts)}`);
        ...
        if (CLOUDFLARE.SLD_EXCEPTIONS.includes(lastTwo)) {
            ...
            console.log(
                `[INFO] Domain with special SLD, taking last 3 parts: ${mainDomain}`
            );
            return mainDomain;
        }

        const mainDomain = parts.slice(-2).join('.');
        console.log(`[INFO] Main domain extracted: ${mainDomain}`);
        return mainDomain;
    }
    ...
    async getZoneId(domain) {
        ...
        console.log(`🔍 Looking up Zone ID for main domain: ${mainDomain}`);
        ...
        try {
            ...
            if (!data.success || !Array.isArray(data.result)) {
                throw new Error('Zone data not found or invalid response.');
            }

            const zone = data.result.find((zone) => zone.name === mainDomain);
            if (!zone) {
                throw new Error(`Zone ID not found for domain: ${mainDomain}`);
            }
    ...
    async createWorker(workerName, hostname) {
        console.log(`🔧 Creating Worker with name: ${workerName}`);
        ...
        try {
            ...
            const data = await response.json();
            if (!data.success) {
                throw new Error(data.errors?.[0]?.message || 'Failed to create Worker.');
            }
    ...
    async addDomainToWorker(workerName, domain) {
        try {
            ...
            console.log(`[DEBUG] Zone ID for ${domain}:`, zoneId);
            ...
            if (!response.data.success) {
                if (response.data.errors?.[0]?.code === 100117 || response.status === 409) {
                    console.log(
                        `ℹ️ Domain ${domain} already configured, continuing...`
                    );
                    return { success: true, domain, message: 'Domain already configured' };
                }
                throw new Error(response.data.errors?.[0]?.message || 'Failed to add domain');
            }
            ...
            if (
                ...
                error.response?.data?.errors?.[0]?.code === 100117
            ) {
                console.log(`ℹ️ Domain ${domain} already configured, continuing...`);
                return { success: true, domain, message: 'Domain already configured' };
            }
    ...
    async addCNAME(hostname) {
        ...
        const cnameRecord = {
            ...
            content: hostname,
            proxied: true,
            comment: 'CNAME for Wildcard by BHMxFT',
        };
        ...
        try {
            ...
            const cnameExists = existingRecords.data.result.some(
                (record) => record.type === 'CNAME' && record.name === `*.${hostname}`
            );

            if (cnameExists) {
                console.log(`ℹ️ CNAME for *.${hostname} already exists, continuing...`);
                return { success: true, message: `CNAME for *.${hostname} already exists.` };
            }
            ...
            if (!response.data.success) {
                throw new Error('Failed to add CNAME: ' + response.data.errors[0].message);
            }

            console.log(`✅ CNAME for *.${hostname} added successfully.`);
            return { success: true, message: `CNAME for *.${hostname} added successfully.` };
        } catch (error) {
            console.error('❌ Error adding CNAME:', error);
            throw new Error(`Failed to add CNAME: ${error.message}`);
        }
    }
    ...
    async deleteWorker(workerName) {
        try {
            ...
            const data = await response.json();
            if (!data.success) {
                console.log(`⚠️ Worker ${workerName} may no longer exist, continuing...`);
                return;
            }

            console.log(`✅ Worker ${workerName} deleted successfully.`);
        } catch (error) {
            console.error(`⚠️ Error deleting worker ${workerName}:`, error.message);
        }
    }
    ...
    async setupwildcard(domain) {
        ...
        try {
            console.log(`⚙️ Setting up wildcard for domain ${domain}`);
            ...
            const domains = [`*.${domain}`, ...allSubdomains.map((sub) => `${sub}.${domain}`)];

            console.log(`📊 Total subdomains to configure: ${domains.length}`);
            ...
            console.log('🧹 Cleaning up temporary worker...');
            ...
            return {
                ...
                message: `Wildcard configured successfully for ${domain}`,
            };
        } catch (error) {
            console.error(`❌ Error setting up wildcard for ${domain}:`, error);
            ...
            throw new Error(`Failed to setup wildcard for ${domain}: ${error.message}`);
        }
    }
}
