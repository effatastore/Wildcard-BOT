const { readConfig, readCustomSubdomains, writeCustomSubdomains } = require('../utils/fileUtils');
...
async function handleListDomain(ctx) {
    ...
    const domainList =
        ...
        (DEFAULT_SUBDOMAINS.length > 0
            ? `🏠 **Default Domain Wildcard:**\n${DEFAULT_SUBDOMAINS.map(
                  (domain, index) => `${index + 1}. ${domain}`
              ).join('\n')}\n\n`
            : '') +
    ...
    if (!query) {
        return ctx.reply(
            '❌ *Format tidak valid!*\n\n' +
                ...
                '💡 Masukkan kata kunci untuk mencari domain yang tersedia.',
            { parse_mode: 'Markdown' }
        );
    }

    try {
        // Get default domains from config
        const config = global.BOT_CONFIG || require('../config/default');
        const defaultDomains = config.DEFAULT_DOMAINS || [
            'support.zoom.us',
            'discord.com',
            'telegram.org',
            'whatsapp.com',
        ...
        // Search in custom domains
        const customSubdomains = readCustomSubdomains();
        const allCustomDomains = Object.values(customSubdomains).flat();

        // Filter results
        const defaultResults = defaultDomains.filter((domain) =>
            domain.toLowerCase().includes(query)
        );
        const customResults = allCustomDomains.filter((domain) =>
            domain.toLowerCase().includes(query)
        );
        ...
        if (results.length === 0) {
            return ctx.reply(
                '🔍 *Hasil Pencarian*\n\n' +
                    `🔎 Kata kunci: "${query}"\n` +
                    '❌ Tidak ditemukan domain yang cocok\n\n' +
                    '💡 *Saran:*\n' +
                    '• Coba kata kunci yang berbeda\n' +
                    '• Gunakan `/listdomain` untuk melihat semua domain\n' +
                    '• Gunakan `/new` untuk menambah domain baru',
                { parse_mode: 'Markdown' }
            );
        }
        ...
        searchResult += '\n💡 Gunakan `/setupwildcard <domain>` untuk konfigurasi.';

        await ctx.reply(searchResult, { parse_mode: 'Markdown' });
    } catch (error) {
        console.error('❌ Error searching domains:', error);
        await ctx.reply(
            '❌ *Terjadi kesalahan saat mencari domain*\n\n' +
                'Silakan coba lagi dalam beberapa menit.',
            { parse_mode: 'Markdown' }
        );
    }
...
async function handleMySub(ctx) {
    ...
    const subdomainList =
        ...
        '🎯 **Daftar subdomain:**\n' +
        mySubdomains.map((domain, index) => `${index + 1}. ${domain}`).join('\n') +
        '\n\n' +
        '⚡ **Quick Actions:**\n' +
...
module.exports = {
    ...
    addCustomDomain: handleNew,
    listDomains: handleListDomain,
    mySubdomains: handleMySub,
    domainAnalytics: handleSearchDomain, // Using search as analytics for now
    clearCache: (ctx) => ctx.reply('🔄 Cache cleared successfully!'), // Simple implementation
};